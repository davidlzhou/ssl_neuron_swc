...
def adjacency_to_neighbors(adjacency):
    ...
    for i, arr in enumerate(adjacency):
        neighbors[i] = set(np.where(np.array(arr) == 1)[0])
    return neighbors

def features_to_neighbors(features):
    neighbors = {i: set() for i in range(len(features))}
    for i, p in enumerate(features.T[6].astype(int)):
        if i != 0:
            neighbors[i].add(p-1)
            neighbors[p-1].add(i)
    return neighbors