...
class GraphDataset(Dataset):
    ...
    @classmethod
    def fromSWC(cls, swc, config, mode='train', inference=False):
        # will ignore data path specified in config file
        self = cls(config)

        count = 0
        for cell_id, features in tqdm(enumerate(swc)):
            neighbors = features_to_neighbors(features)
            soma_id = 0
    ...
    @classmethod
    def fromCells(cls, config, mode='train', inference=False):
        self = cls(config)
...
def build_dataloader(config, use_cuda=torch.cuda.is_available(), cells_or_SWC='cells', swc=None):

    kwargs = {'num_workers':config['data']['num_workers'], 'pin_memory':True, 'persistent_workers': True} if use_cuda else {}

    if cells_or_SWC == 'SWC':
        train_loader = DataLoader(
                GraphDataset.fromSWC(swc=swc, config=config, mode='train'),
                batch_size=config['data']['batch_size'], 
                shuffle=True, 
                drop_last=True,
                **kwargs)

        val_dataset = GraphDataset.fromSWC(swc=swc, config=config, mode='val')
        ...
        train_loader = DataLoader(
                GraphDataset.fromCells(config, mode='train'),
                batch_size=config['data']['batch_size'], 
                shuffle=True, 
                drop_last=True,
                **kwargs)

        val_dataset = GraphDataset.fromCells(config, mode='val')
        batch_size = val_dataset.num_samples if val_dataset.__len__() < config['data']['batch_size'] else config['data']['batch_size']
        val_loader = DataLoader(
                val_dataset,
                batch_size=batch_size,
...
def build_train_loader(config, use_cuda=torch.cuda.is_available(), swc=None):
    kwargs = {'num_workers':config['data']['num_workers'], 'pin_memory':True, 'persistent_workers': True} if use_cuda else {}
...
def build_val_loader(config, use_cuda=torch.cuda.is_available(), swc=None):
